...
document.getElementById('logoutBtn').addEventListener('click', () => {
  ...
  customConfirm('Willst du dich wirklich ausloggen?', (confirmed) => {
    if (confirmed) {
      localStorage.clear();
      window.location.href = '/';
    }
...
socket.on('chat message', (message) => {
  ...
  if (roomNotificationPrefs[currentRoom] && document.hidden) {
    new Notification(`Neue Nachricht in ${room}`, {
      body: `${message.username}: ${message.text || "Neue Nachricht"}`,
      icon: message.profilePic, // Optional
    });
  }
...
customConfirm('Willst du wirklich deinen Account und alle deine Nachrichten löschen? Diese Aktion ist unwiderruflich!', (confirmed) => {
  if (confirmed) {
    socket.emit('delete account', { username: currentUsername });
  }
});
...
function editMessage(id, oldText) {
  ...
  // }
  customPrompt('Neue Nachricht:', oldText, (newText) => {
    if (newText && newText !== oldText) {
      socket.emit('edit message', { id, newText, room: currentRoom });
    }
  });
...
function deleteMessage(id) {
  ...
  customConfirm('Nachricht wirklich löschen?', (confirmed) => {
    if (confirmed) {
      socket.emit('delete message', { id, room: currentRoom });
    }
  });
...
document.getElementById('reportBugBtn').addEventListener('click', () => {
  document.getElementById('bugDialog').style.display = 'block';
});

document.getElementById('cancelBug').addEventListener('click', () => {
  document.getElementById('bugDialog').style.display = 'none';
...
document.getElementById('sendBug').addEventListener('click', () => {
  ...
  if (message.length === 0 || message.length > 500) {
    alert('Bitte eine gültige Nachricht (max. 500 Zeichen) eingeben.');
    return;
  }
  ...
  fetch('/report-bug', {
    ...
    body: JSON.stringify({ username: currentUsername, message }) // `username` muss verfügbar sein
  })
    .then(res => res.json())
    .then(data => {
